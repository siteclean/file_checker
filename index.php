...
if( isset($_GET['check']) and $_GET['pass'] == SC_file_checker_password )
{
	...
	$data_file = get_option('filechecker_save_dir').'/FC.'.'datafile';
	SC_filechecker_auto_check($path, $data_file);	
	exit();
}
...
$path = esc_textarea(get_option('filechecker_scan_dir'));
$path_for_backups = esc_textarea(get_option('filechecker_path_for_backup'));
$data_file = esc_textarea(get_option('filechecker_save_dir')).'/'.'FC.'.'datafile';
...
function SC_filechecker_settings() 
{
		...
		and wp_verify_nonce($_POST['_wpnonce'],'update_settings') )
	{
				...
				$scan_dir = esc_textarea(SC_filechecker_clear_dir_path($_POST['scan_dir']));
				if(!empty($scan_dir))
				{
	...
	if(isset($_POST['rescan']) and SC_filechecker_check_password($_POST['FC_password']) !== FALSE and wp_verify_nonce($_POST['_wpnonce'],'update_settings') )
	{
		...
		SC_filechecker_create_data_file($path, $data_file);
	}
